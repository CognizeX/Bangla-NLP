...
def process_text():
    ...
    audio = bangla_tts(model=tts_model, text=text, is_male=False, is_e2e_vits=True, log_dir=save_dir)

    logging.info(f"Generated audio file: {audio}")
    response = {
        "audio_url": audio,
        "sender": sender,
        "status": 200,
...
if __name__ == "__main__":
    app.run(debug=True, host="192.168.0.1", port=8000)
